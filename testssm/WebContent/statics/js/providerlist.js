	...
	$.ajax({
		...
		success:function(data){
			if(data.result == "true"){//删除成功：移除删除行
				...
				alert(data);
				//alert("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.result+"】条订单，不能删除");
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.result+"】条订单，不能删除");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/pro/proview/"+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/pro/proupdate/"+ obj.attr("proid");
	});
