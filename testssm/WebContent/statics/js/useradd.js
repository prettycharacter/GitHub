...
$(function(){
	...
	_idPicPath=$("#_idPicPath");
	_workPicPath=$("#_workPicPath");
	...
	birthday.next().html("*");
	userRole.next().html("*");
	_idPicPath.next().html("*");
	_workPicPath.next().html("*");
	
	if(uploadError.val()==null||uploadError.val()==""){
		_idPicPath.next().html("文件上传大小不能超过500kB,上传文件类型为jpg,jpeg,png,pneg");
		_workPicPath.next().html("文件上传大小不能超过500kB,上传文件类型为jpg,jpeg,png,pneg");
	}else{
	_idPicPath.next().html(uploadError.val());
	_workPicPath.next().html(uploadError.val());
	}
	
	$.ajax({
		type:"GET",//请求类型
		url:path+"/user/getuserrolelist",//请求的url
		data:null,//请求参数
	...
	userCode.bind("blur",function(){
		...
		if(userCode.val()==""||userCode.val()==null){
			validateTip(userCode.next(),{"color":"red"},imgNo+ "用户名不能为空",false);
		}else{
		$.ajax({
			type:"GET",//请求类型
			url:path+"/user/ucexits",//请求的url
